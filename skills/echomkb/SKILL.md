---
name: echomkb
description: Midnight Network version + citation layer for coding agents — which @midnight-ntwrk / Compact versions the support matrix supports per network vs what is newest on relnotes/npm/GitHub, official docs pages as markdown with their language/compiler stamp, a citable URL for any Midnight claim, and a live search over the upstream issue trackers with a method for reporting what the network did — after the human approves. Use when pinning versions, when a Midnight claim needs a source URL, or when observed network behavior contradicts the docs. For "what do the docs say", prefer the official Kapa MCP server when the session has it; this skill is the loud fallback.
---

# EchoMKB — the layer under the docs

Your training data about Midnight and Compact is stale and partly wrong. This skill exists so you never answer from memory: every Midnight fact you state carries the URL it came from, every version you pin comes from the support matrix, and when the network does something the docs don't describe, you search the upstream trackers before you debug and report upstream — with the human's approval — instead of guessing.

Requires Node ≥ 18 (global `fetch`). No dependencies. Scripts live next to this file in `scripts/`. Contacts only `docs.midnight.network`, plus `registry.npmjs.org` (via `npm view`) and `api.github.com` for the `versions` and `issues` commands, and one anonymous handshake to `midnight.mcp.kapa.ai` for `doctor`; nothing from the user's project is sent anywhere.
...
| "The docs say X but the network does Y" | `issues <words>` — live GitHub issue search over `midnightntwrk/*`, `input-output-hk/lace` and `LFDT-Minokawa/compact`; then the reporting method in step 6. |
...
6. **When behavior contradicts the docs — Report it.** This is the step the other tools stop before. Run
   `node <skill-dir>/scripts/echomkb.mjs issues "<symptom, error code, component>"`
   before debugging from scratch and before filing anything: the failure may already be known, and a comment with your environment and observation on an existing issue is worth more than a duplicate. If nothing matches:
   - **Pin the environment** — network · node/indexer versions · wallet-SDK + dust-wallet versions · whether the failing circuit reads or writes ledger state.
   - **Reproduce on a second network.** When the same asymmetry shows up on two networks, suspect the mechanism they share before the code that differs.
   - **Search again** with the exact error text or code.
   - **Write the report** — environment · reproduction steps · evidence (logs, tx hashes, proof-server output) · what you ruled out · what the SDK or docs should do · open questions. `gh issue create` on `midnightntwrk/*` or `input-output-hk/lace` (or `gh issue comment` on the closest existing issue) is fine, **but only after the human has read the final text and explicitly authorized it** — never on your own initiative; it is submitted under their name.
...
| `issues <words> [--max 8] [--json]` | Live GitHub issue search (`org:midnightntwrk`, `input-output-hk/lace`, `LFDT-Minokawa/compact`, newest first). Anonymous GitHub search allows 10 requests/min. |
