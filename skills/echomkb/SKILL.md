...
| "The docs say X but the network does Y" | `issues <words>` — live GitHub issue search over `midnightntwrk/*`, `input-output-hk/lace` and `LFDT-Minokawa/compact`, plus any matching `OBSERVATIONS.md` entry (workaround, environment) with the issue's live state. The ledger is read from GitHub at run time; the installed skill carries no copy. |
...
6. **When behavior contradicts the docs**, run
   `node <skill-dir>/scripts/echomkb.mjs issues "<symptom, error code, component>"`
   before debugging from scratch. It searches the upstream trackers live and prints any matching `OBSERVATIONS.md` entry with its workaround and the issue's current state — the failure may already be known, and a duplicate issue helps nobody. If nothing matches, follow the method section at the top of `OBSERVATIONS.md` (pin the environment quadruple → reproduce on a second network → search existing issues → report upstream) and propose a new entry. You may draft the upstream issue and file it yourself (`gh issue create` on `midnightntwrk/*` or `input-output-hk/lace`), **but only after the human has read the final text and explicitly authorized filing** — never on your own initiative; the issue is submitted under their name.
...
| `issues <words> [--max 8] [--json]` | Live GitHub issue search (`org:midnightntwrk`, `input-output-hk/lace`, `LFDT-Minokawa/compact`, newest first) plus matching `OBSERVATIONS.md` entries with workaround and live tracker state. Anonymous GitHub search allows 10 requests/min. |
